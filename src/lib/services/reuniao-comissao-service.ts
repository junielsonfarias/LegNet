...
export class ReuniaoComissaoService {
  ...
  /**
   * Adicionar multiplas proposicoes a pauta de uma vez
   */
  static async adicionarItensPautaBulk(reuniaoId: string, proposicaoIds: string[]) {
    // Verificar se reuniao existe
    const reuniao = await prisma.reuniaoComissao.findUnique({
      where: { id: reuniaoId }
    })
    ...
    if (itensParaCriar.length === 0) {
      return {
        itens: [],
        adicionados: 0,
        jaExistentes: proposicoes.length
      }
    }

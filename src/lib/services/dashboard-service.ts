...
export const dashboardService = {
  ...
  async getAtividades(limit: number = 10): Promise<AtividadeRecente[]> {
    ...
    // Cada seção protegida individualmente para não derrubar tudo se uma falhar
    try {
      ...
      proposicoesRecentes.forEach(p => {
        atividades.push({
          ...
          description: p.titulo || (p.ementa ? p.ementa.substring(0, 60) + '...' : 'Sem descrição'),

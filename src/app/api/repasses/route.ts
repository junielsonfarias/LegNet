...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (params.orgaoOrigem) where.orgaoOrigem = { contains: params.orgaoOrigem, mode: 'insensitive' }
...
export const POST = withAuth(async (request: NextRequest) => {
  ...
  const created = await prisma.repasse.create({
    data: { ...data, data: new Date(data.data) }
  })
  return createSuccessResponse(created, 'Repasse criado', undefined, 201)

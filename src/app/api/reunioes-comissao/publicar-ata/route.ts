 ...
 *
 * Espelha RN-170 (Ata de Sessao) para reunioes de comissao. A ata vive em
 * `ReuniaoComissao.arquivoAta` + `ReuniaoComissao.dataPublicacaoAta`.
 * Sem fonte paralela em Publicacao.
 *
 * Dois modos:
 *  1. `reuniaoId` informado -> anexa ata em reuniao existente.
 *  2. `comissaoId + numero + ano + tipo` informados -> find-or-create.
 *     Se nao achar ReuniaoComissao com (comissaoId, numero, ano), cria
 *     nova com `status=CONCLUIDA, ataAprovada=true`.
 *
  ...
  withErrorHandler(async (request: NextRequest, _context, session: Session) => {
    ...
    if (role !== 'ADMIN' && role !== 'SECRETARIA') {
      throw new UnauthorizedError(
        'Apenas ADMIN ou SECRETARIA podem publicar atas de reuniao',
      )
    }
    ...
    if (payload.reuniaoId) {
      ...
      // findFirst por (comissaoId, numero, ano) — eh o unique do model
      ...
      if (found) {
        ...
        const nova = await prisma.reuniaoComissao.create({
          data: {
            ...
            ataAprovada: true,
            dataAprovacaoAta: dataPublicacao,
    ...
    await logAudit({
      request,
      session,
      action: 'REUNIAO_COMISSAO_ATA_PUBLICACAO',
    ...
    return createSuccessResponse(
      ...
      reuniaoCriada
        ? 'Reuniao criada e ata publicada com sucesso'

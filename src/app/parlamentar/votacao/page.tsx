...
export default function VotacaoParlamentarPage() {
  ...
  // Dados do parlamentar e cidade para o header
  const nomeParlamentar = parlamentarInfo?.apelido || parlamentarInfo?.nome || (session?.user as any)?.name || 'Parlamentar'
  const iniciais = nomeParlamentar.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()
  const cidade = configuracao.endereco?.cidade || 'Mojuí dos Campos'
  ...
  return (
    <div className="h-[100dvh] bg-gray-50 flex flex-col overflow-hidden">
      ...
      <header className="flex-shrink-0 bg-gradient-to-r from-blue-900 via-blue-800 to-blue-900 text-white">
        {/* Barra superior - Câmara */}
        <div className="border-b border-white/10 px-3 sm:px-4 py-2 sm:py-2.5">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              {configuracao.logoUrl ? (
                <Image
                  ...
                  width={36}
                  height={36}
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-contain bg-white p-0.5"
                  unoptimized
                />
              ) : (
            ...
            <div className="flex flex-col items-end gap-0.5">
              <div className="flex items-center gap-1.5 text-[10px] sm:text-xs text-blue-100">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[9px] sm:text-[10px] text-blue-300">
                  {formatarDataSessao(sessaoAtiva.data)}
                </span>
                {sessaoAtiva.tempoInicio && (
                  <div className="flex items-center gap-1 font-mono text-[10px] sm:text-xs text-green-300 font-semibold">
                    <Timer className="h-3 w-3" />
                    {formatarTempo(tempoSessao)}
                  </div>
                )}

...
server {
    ...
    # Headers de segurança
    # NOTA: Content-Security-Policy e Strict-Transport-Security sao enviados
    # pelo middleware do Next.js (src/middleware.ts) para evitar conflito de
    # duas politicas CSP (navegador aplica intersecao = mais restritiva,
    # bloqueando VLibras e outros recursos legitimos).
    add_header X-Frame-Options "SAMEORIGIN" always;
    add_header X-Content-Type-Options "nosniff" always;
    add_header X-XSS-Protection "1; mode=block" always;
    add_header Referrer-Policy "strict-origin-when-cross-origin" always;

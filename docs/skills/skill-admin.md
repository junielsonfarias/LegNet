...
| **RN-144** | 2FA OBRIGATORIO para roles ADMIN e SECRETARIA (Fase 1/C4) — CONDICIONAL a politica global. Quando `Configuracao.seguranca.2fa.enabled = true`, middleware redireciona usuarios sem 2FA para /admin/configuracoes/seguranca?enroll=1; quando `false`, ninguem e forcado e o codigo TOTP nao e exigido no login mesmo para usuarios com 2FA configurado. JWT atualizado via session.update() apos habilitar/desabilitar para refletir mudanca sem novo login. Toggle global via `GET/PUT /api/admin/configuracoes/2fa` (ADMIN + config.manage); helper `src/lib/security/two-factor-config.ts` com cache TTL 60s. |

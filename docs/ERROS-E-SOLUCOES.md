...
| ID | Problema | Solução |
|----|----------|---------|
| ERR-071 | O **Texto Integral** da proposição (campo `texto`, OCR de PDF escaneado na migração) exibia **ruído físico do papel**: carimbo de protocolo ("Protocolo sob nº... O)", "! OTOCOL"), marca d'água ("Digitalizado com CamScanner"), rabisco da assinatura ("/ : |) é"), cabeçalho/rodapé. O corpo do documento estava intacto. Limpeza agressiva era **perigosa** (RN-004): quando o lixo grudava na mesma linha do texto real (título/autor), remover a linha apagava conteúdo oficial. | `limparTextoIntegralOCR` em `legislative-labels.ts`: limpador CONSERVADOR **só no display** (não altera o dado — preserva o original). Remove apenas linhas INTEIRAMENTE lixo (carimbo/marca d'água/rabisco ancorados; linhas curtas com <50% de letras) e NUNCA toca em linhas com palavra de conteúdo (protege título/autor/corpo). Resíduo inline aceito de propósito. Aplicado no detalhe da proposição + rótulo "Transcrição automática (OCR)" e link para o documento original (fonte oficial). |
| ERR-070 | Listagens de matérias/votações com dois defeitos: (1) **ementas exibiam `$n^{0}$`** (notação LaTeX de "nº" vinda do OCR/migração), ex.: "O Projeto de Lei $n^{0}$ 011/2024"; (2) **ordem errada** — a API entrega por `dataApresentacao desc` e `numero` é String com padding não-uniforme no acervo (ex.: "9" e "060" convivem), então a lista aparecia embaralhada por número. Além disso a home repetia o mesmo PL várias vezes (a API de votações devolve 1 linha por voto). | `src/lib/utils/legislative-labels.ts`: `normalizarTextoLegislativo` (regex `\$?([A-Za-z0-9])\^\{?([oO0aA])\}?\$?` → Xº/Xª, sem comer espaços vizinhos; ignora "R$ 500" e "m^2") + `ordenarMaterias`/`parseNumeroMateria` (ordena por ano desc + número **inteiro** desc). Aplicado em `/legislativo/proposicoes`, `votacoes-nominais`, home widget e detalhe da proposição. Home dedup por proposição. Script `prisma/scripts/limpa-artefatos-texto.ts` limpa a fonte (Proposicao/NormaJuridica) — idempotente, dry-run/--apply. |
| ERR-069 | Perfil do parlamentar (`/parlamentares/[slug]`) e dashboards **agregavam presença e produção de TODOS os mandatos** e rotulavam como "Legislatura Atual". Um vereador com 2 mandatos (ex.: José Martins — 2025-2028 ativo + 2021-2024 encerrado) tinha frequência/matérias **somadas de 2021 a 2028** no card "atual". Sem separação por mandato; e a legislatura 2021-2024 exibia `-1ª` (número sentinela da migração). | **Backend** `parlamentar-db-service`: novo helper `computeMandatosBreakdown` segmenta presença (ano de `sessao.data`) e produção (`proposicao.ano`) pela faixa `anoInicio–anoFim` da legislatura de cada mandato. `getPerfil`/`getDashboard`: estatística do topo passa a refletir **só a legislatura ativa** + novo `mandatosDetalhados[]` (presença + produção por mandato). **Frontend**: card do topo rotulado com a legislatura ativa; aba "Mandatos" mostra frequência + material produzido **por mandato** (ativo destacado + "Mandatos Anteriores"); dashboard do parlamentar idem. **Dados**: `formatLegislaturaLabel` some com o `-1ª` na exibição; script `prisma/scripts/renumera-legislaturas.ts` + importers 01/29 renumeram para os números reais (2025-2028=18ª … 2013-2016=15ª). |
